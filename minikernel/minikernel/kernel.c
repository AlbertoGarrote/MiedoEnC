...
// Tratamiento para obtener ID

int sis_obtenerID(){

	int id = p_proc_actual->id;
	printk("-> EL RPOCESO %d OBTIENE SU ID", p_proc_actual->id);
	return id;
...
int sis_dormir(){
	...
	BCP* procesoDormido = p_proc_actual;

	procesoDormido->tiempo_dormir = segundos * TICK;

	eliminar_primero(&lista_listos);
	insertar_ultimo(&lista_dormidos, procesoDormido);

	p_proc_actual = planificador();

	cambio_contexto(&(procesoDormido->contexto_regs), &(p_proc_actual->contexto_regs));
